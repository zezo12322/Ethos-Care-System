      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {news.length === 0 ? <p className="text-outline">لا توجد أخبار حالياً</p> : news.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl overflow-hidden border border-outline-variant/20 group hover:shadow-lg transition-all">
              <div className="h-48 relative overflow-hidden bg-surface-container-low flex items-center justify-center">
                {item.image ? (
                  <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
                  <span className="material-symbols-outlined text-4xl text-outline">newspaper</span>
                )}
                <div className="absolute top-4 right-4 bg-[#fcb900] text-xs font-bold px-3 py-1 rounded-full">{item.category}</div>
              </div>
              <div className="p-6">
