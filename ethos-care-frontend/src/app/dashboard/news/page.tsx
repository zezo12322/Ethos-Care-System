...
export default function NewsPage() {
  ...
  const [formData, setFormData] = useState({ title: '', category: 'فعاليات', content: '', image: '', status: 'نشط' });

  useEffect(() => { fetchNews(); }, []);

  const fetchNews = () => {
    setLoading(true);
    api.get('/news').then(res => { setNews(res.data); setLoading(false); })
  ...
  const handleCreateNewsItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ title: '', category: 'فعاليات', content: '', image: '', status: 'نشط' });
      fetchNews();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center gap-4">
        <div><h1 className="text-3xl font-bold font-headline text-on-surface">إدارة الأخبار</h1></div>
      </div>
      <div className="bg-white rounded-2xl border border-outline-variant/30 overflow-hidden">
        <div className="p-4 border-b border-outline-variant/30 flex justify-between items-center">
          <h2 className="font-bold text-lg">الأخبار والمؤسسات</h2>
          <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg font-bold text-sm">خبر جديد</button>
        </div>
          ...
          <table className="w-full text-right border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/20 text-sm font-medium">
              {loading ? <tr><td colSpan={5} className="p-6 text-center text-outline">جار التحميل...</td></tr> :
               news.length === 0 ? <tr><td colSpan={5} className="p-6 text-center text-outline">لا يوجد بيانات للشركاء</td></tr> :
               news.map((p) => (
                 <tr key={p.id}>
                   <td className="px-6 py-4 font-bold">{p.title}</td>
                   <td className="px-6 py-4">{p.category}</td>
                   <td className="px-6 py-4 text-on-surface-variant font-mono">{p.content} <br/><span className="text-xs">{p.image}</span></td>
                   ...
                   <td className="px-6 py-4 text-center">
                     <button onClick={() => handleDeleteNewsItem(p.id)} className="w-8 h-8 rounded-full text-red-600 hover:bg-red-100"><span className="material-symbols-outlined text-[18px]">delete</span></button>
                   </td>
                 </tr>
               ))}
            </tbody>
