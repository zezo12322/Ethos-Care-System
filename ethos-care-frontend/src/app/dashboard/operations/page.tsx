...
export default function OperationsPage() {
  ...
  const [formData, setFormData] = useState({ name: "", type: "حملة توزيع إطعام", target: 100, volunteers: 10 });
  ...
  const handleCreateOperation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: "", type: "حملة توزيع إطعام", target: 100, volunteers: 10 });
      fetchOperations();
    } catch (error) {
      console.error(error);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div key={op.id} className="bg-white rounded-3xl p-6 border border-outline-variant/30 shadow-[0px_4px_16px_-4px_rgba(0,40,38,0.04)] hover:shadow-lg transition-shadow flex flex-col">
            ...
            <h3 className="text-xl font-bold font-headline mb-1 text-on-surface">{op.title || op.name}</h3>
            ...
            <div className="mt-auto space-y-4">
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  ...
                  <span className="text-primary">{op.progress || 0}%</span>
                </div>
                <div className="w-full bg-surface-container-highest rounded-full h-2 overflow-hidden">
                  <div className="bg-primary h-full rounded-full" style={{ width: `${op.progress || 0}%` }}></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-4 border-t border-outline-variant/20">
                <div>
                  ...
                  <p className="font-bold text-lg"><span className="text-primary">{op.achieved || 0}</span> / {op.targetFamilies || op.target}</p>
                </div>
                <div>
                  <p className="text-xs text-on-surface-variant">فريق التطوع</p>
                  <p className="font-bold text-lg">{op.volunteers || 0} متطوع</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl leading-relaxed text-right">
            ...
            <form onSubmit={handleCreateOperation} className="space-y-4">
              <input required type="text" placeholder="اسم العملية أو الحملة" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full border border-outline-variant/50 rounded-xl py-2 px-3 outline-none focus:border-primary" />
              ...
              <div className="flex gap-2">
                <input required type="number" placeholder="المستهدف" value={formData.target} onChange={e => setFormData({...formData, target: parseInt(e.target.value) || 0})} className="w-1/2 border border-outline-variant/50 rounded-xl py-2 px-3 outline-none focus:border-primary" />
                <input required type="number" placeholder="عدد المتطوعين المتاح" value={formData.volunteers} onChange={e => setFormData({...formData, volunteers: parseInt(e.target.value) || 0})} className="w-1/2 border border-outline-variant/50 rounded-xl py-2 px-3 outline-none focus:border-primary" />
              </div>
